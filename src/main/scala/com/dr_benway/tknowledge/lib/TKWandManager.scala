...
object TKWandManager extends IWandTriggerManager {
  ...
  private def fitFountain(world: World, pos: BlockPos): Boolean = {
    ...
    val blueprint_t = List(
        ...
        Point(-2, -1, 0)  -> b0, Point(-1, -1, 0)  -> b5, Point(0, -1, 0)  -> b4, Point(1, -1, 0)  -> b5, Point(2, -1, 0)  -> b0,
        Point(-2, -1, 1)  -> b0, Point(-1, -1, 1)  -> b5, Point(0, -1, 1)  -> b5, Point(1, -1, 1)  -> b5, Point(2, -1, 1)  -> b0,
        Point(-2, -1, 2)  -> b0, Point(-1, -1, 2)  -> b0, Point(0, -1, 2)  -> b0, Point(1, -1, 2)  -> b0, Point(2, -1, 2)  -> b0
    ) 
  ...
  private def placeFountain(world: World, pos: BlockPos): Boolean = {
    ...
    setBlockSparkle(world, pos.add(0, -3, 0), TKBlocks.fountain.getDefaultState.withProperty(TKBlocks.fountain.TYPE, EnumProperty.EnumFountainType.PILLAR_BOTTOM))
