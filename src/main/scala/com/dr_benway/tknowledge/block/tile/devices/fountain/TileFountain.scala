...
class TileFountain extends BaseTE with ISidedInventory with IAspectContainer with IFluidHandler with IEssentiaTransport with ITickable {
  ...
  private def craftCycle() {
    ...
    enoughE = true
    if(currentRecipe.getEssentia.visSize() > 0)
      drawEssentia()
    if(currentRecipe.getVis.visSize() > 0 && enoughE == true)
      drawVis()
    if((enoughE == true || currentRecipe.getEssentia.visSize() <= 0) && this.vis.visSize() >= currentRecipe.getVis.visSize())
      endRecipe()
  }
  ...
  private def drawEssentia() {
     ...
     this.currentRecipe.getEssentia.getAspectsSortedByAmount.toList find { x => this.essentia.getAmount(x) < this.currentRecipe.getEssentia.getAmount(x) } match {
       case Some(a: Aspect) => { enoughE = false; currentSuction = a }
       case None => currentSuction = null
     }
      
     if(currentSuction != null && enoughE == false)  {
      for(dir <- EnumFacing.VALUES) {
        if(dir != EnumFacing.UP && dir != EnumFacing.DOWN) {
          ...
          if(te != null) {
            ...
            if((ic.getEssentiaAmount(dir.getOpposite) > 0) && ic.getEssentiaType(dir.getOpposite) == this.currentSuction && (ic.getSuctionAmount(dir.getOpposite) < slave.getSuctionAmount(null)) && (slave.getSuctionAmount(null) >= ic.getMinimumSuction) && this.essentia.getAmount(currentSuction) < currentRecipe.getEssentia.getAmount(currentSuction) * eCost) {
  ...
  private def drawVis() {
    ...
    for(a <- currentRecipe.getVis.getAspectsSortedByAmount) {
    if(this.vis.getAmount(a) < currentRecipe.getVis.getAmount(a) * vCost) {
  ...
  private def endRecipe() {
    ...
    if(this.tank.getFluidAmount == 0 || (this.tank.getFluid.isFluidEqual(currentRecipe.getResult) && ( (currentRecipe.getResult.getFluid.getTemperature >= 1500 && this.tank.getCapacity - this.tank.getFluidAmount >= currentRecipe.getResult.amount * efficiency) || (currentRecipe.getResult.getFluid.getTemperature < 1500 && this.tank.getCapacity - this.tank.getFluidAmount >= currentRecipe.getResult.amount * (2 - efficiency)))) ) {
      
      if((this.getStackInSlot(0) != null && OreDictionary.itemMatches(currentRecipe.getCatalyst.asInstanceOf[ItemStack], this.getStackInSlot(0), false)) || (currentRecipe.getCatalyst == null)) {
        
        if((currentRecipe.getEssentia.visSize() > 0 && tick % ((currentRecipe.getEssentia.visSize() + currentRecipe.getVis.visSize()) * 3 * speed ).toInt == 0) || (currentRecipe.getEssentia.visSize() == 0 && tick % (20 * speed) == 0) ) {
        
        if(currentRecipe.getCatalyst != null)
          this.setInventorySlotContents(0, null)
        
        for(a <- currentRecipe.getEssentia.getAspectsSortedByAmount) {
          this.essentia.remove(a, (currentRecipe.getEssentia.getAmount(a) * eCost).toInt)
        }
        ...
        if(currentRecipe.getResult.getFluid.getTemperature >= 1500)
          this.fill(EnumFacing.UP, new FluidStack(currentRecipe.getResult.getFluid, (currentRecipe.getResult.amount * efficiency).toInt), true) else 
            this.fill(EnumFacing.UP, new FluidStack(currentRecipe.getResult.getFluid, (currentRecipe.getResult.amount * (2 - efficiency)).toInt), true)  
        }
  ...
  override def writeCustomNBT(nbt: NBTTagCompound) {
    ...
    if(locked == true)
      nbt.setString("key", currentRecipe.getKey)
  }
  ...
  override def addToContainer(aspect: Aspect, i: Int) = {
    val ce = this.currentRecipe.getEssentia.getAmount(aspect) - this.essentia.getAmount(aspect)
 ...
 //Fluid handling
  ...
  def calcCupHeight() = if(currentRecipe.getEssentia.visSize() > 0) ( MIN_CUP_HEIGHT + ((MAX_CUP_HEIGHT - MIN_CUP_HEIGHT) / (currentRecipe.getEssentia.visSize() + currentRecipe.getVis.visSize())) * (this.essentia.visSize() + this.vis.visSize()) ) else MAX_CUP_HEIGHT
