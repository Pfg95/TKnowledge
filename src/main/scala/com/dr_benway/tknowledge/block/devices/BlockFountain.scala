...
import com.dr_benway.tknowledge.block.tile.devices.TileFountainFluidHolder
...
import com.dr_benway.tknowledge.block.tile.devices.TileFountainEssentiaHolder
...
class BlockFountain(uName: String = "wat", mat: Material = Material.rock, hardness: Float = 2.0f, resistance: Float = 2.0f) extends {
    ...
    setDefaultState(this.getBlockState.getBaseState.withProperty(TYPE, EnumProperty.EnumFountainType.PILLAR_BOTTOM).withProperty(NORTH, Boolean.box(false)).withProperty(SOUTH, Boolean.box(false)).withProperty(EAST, Boolean.box(false)).withProperty(WEST, Boolean.box(false)))
    
    override def getSpecialName(stack: ItemStack): String = stack.getItemDamage match {
      case 0 => "pillar_bottom"
    ...
    override def getStateFromMeta(meta: Int): IBlockState = {
      getDefaultState().withProperty(TYPE, meta match {
        case 0 => EnumProperty.EnumFountainType.PILLAR_BOTTOM
    ...
    private def hasNextBlock(state: IBlockState, worldIn: IBlockAccess, pos: BlockPos) = {
      Boolean.box(!(worldIn.getBlockState(pos).getBlock.isInstanceOf[BlockFountain]) && ((getMetaFromState(state)) == 6 || getMetaFromState(state) == 3 || getMetaFromState(state) == 7 || getMetaFromState(state) == 8))
    }
    
    private def hasTop(state: IBlockState, worldIn: IBlockAccess, pos: BlockPos) = getMetaFromState(worldIn.getBlockState(pos.up)) == 2
    ...
    override def onBlockActivated(world: World, pos: BlockPos, state: IBlockState, player: EntityPlayer, side: EnumFacing, hitX: Float, hitY: Float, hitZ: Float) = {
      ...
      if(world.isRemote) true
        ...
        if(tile != null && tile.isInstanceOf[TileFountain]) {
          val cont = tile.asInstanceOf[TileFountain]
        ...
        if(!player.isSneaking()) {
          ...
          if(fs != null) {
            if((master.tank.getFluidAmount < master.tank.getCapacity) && (master.tank.getFluid == null || master.tank.getFluid.isFluidEqual(fs))) {
              master.fill(EnumFacing.UP, FluidContainerRegistry.getFluidForFilledItem(player.inventory.getCurrentItem()), true)
  ...
  override def onWandRightClick(world: World, is: ItemStack, eplayer: EntityPlayer, pos: BlockPos, facing: EnumFacing) = {
    ...
    if(!world.isRemote && !eplayer.isSneaking() && this.getMetaFromState(world.getBlockState(pos)) == 5) {
      ...
      //if(master.locked == false && master.essentia.visSize() == 0) master.locked = true else master.locked = false
